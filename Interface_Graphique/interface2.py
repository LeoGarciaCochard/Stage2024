...
likert_stress = Likert(frame_quest_participant,dic_stress,selected_var_stress, text= dico_text["stress"])
...
button_part2.pack(pady=(0,50), padx=10, side=tk.BOTTOM)
...
def versBouton() :
    ...
    print(dic_bruit[likert_bruit.slider.slider.get()])
    ...
    if validate_variables() :
        ...
        nouvelle_ligne ={
            ...
            "Maîtrise de l'informatique"  : selected_var_hability_inf[0],
            "Experience" : selected_var_experience,
            "Passion"  : selected_var_passion[0],
            "Bruit"  : selected_var_bruit[0],
            "Tâche" : selected_var_tache
        }
...
def envoyer_en_l_etat() :
    ...
    niveau_importance = dic_importance[likert_importance.slider.slider.get().slider.get()]
...
def Type_Concentration():
    ...
    likert_importance.cadre.pack_forget()
    frame_questCommentaire.pack_forget()

    button_Type_Concentration.pack_forget()

    button_Concentration_Type.pack(padx=10, pady=10, anchor='center')

    likert_concentration.cadre.pack(pady=5, anchor='center', expand=True)
    frame_questDistraction.pack(pady=5, anchor='center', expand=True)
    likert_fatigue.cadre.pack(pady=5, anchor='center', expand=True)
    frame_questDifficulte.pack(pady=5, anchor='center', expand=True)
    button_done.pack(pady=20, padx=10, anchor='center', expand=True)

    if button_quit_Modif.winfo_ismapped() :
        button_done.pack_forget()
        button_done_modif.pack(pady=20, padx=10, anchor='center')
...
def Concentration_Type(rep = 0):
    ...
    likert_fatigue.cadre.pack_forget()
    ...
    frame_questType.pack(pady=5, expand=True)
    likert_importance.cadre.pack(pady=5, expand=True, anchor='center')
    frame_questCommentaire.pack(pady=5, expand=True)
...
frame_quest = ctk.CTkFrame(master=root, fg_color="#333333")
...
frame_questType = ctk.CTkFrame(master=frame_quest, fg_color="#333333")
...
frame_questDistraction = ctk.CTkFrame(master = frame_quest, fg_color="#333333")
...
#   Likert Importance/Gravité
    ...
    likert_importance.slider.slider.set(list_val[2])
...
#   Likert Concentration
    ...
    likert_concentration.slider.slider.set(list_val[4])
...
#   Likert Fatigue
    ...
    likert_fatigue.slider.slider.set(list_val[7])
...
def sauvegarderQuest() :
    ...
    type = get_selected_button_value()
    faute = selected_option_Bad.get()
    niveau_importance = dic_importance[likert_importance.slider.slider.get()]
    description = entry_Commentaire.get("1.0", tk.END).strip()
    niveau_concentration = dic_concentration[likert_concentration.slider.slider.get()]
    distraction = selected_optionDistraction.get()
    natureDistration = entry_Distraction.get()
    niveau_fatigue = dic_fatigue[likert_concentration.slider.slider.get()]
    niveau_difficulte = selected_optionDifficulte.get()
